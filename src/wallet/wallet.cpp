...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/vhkd/vhkd/pull/10340, but that change has been
    // postponed until after https://github.com/vhkd/vhkd/pull/10244 to
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-vhkd-address
            ...
            while (true)
            {
	         ...
	         printf("[%04d]:%d, nValueToSelect = %lld\n", nindex++, __LINE__, nValueToSelect);
			 
                if (nSubtractFeeFromAmount == 0)
                    nValueToSelect += nFeeRet;

		 printf("[%04d]:%d, nValueToSelect = %lld\n", nindex++, __LINE__, nValueToSelect);
		 		 printf("[%04d]:%d, nFeeRet = %lld\n", nindex++, __LINE__, nFeeRet);
				 		 printf("[%04d]:%d, nSubtractFeeFromAmount = %d\n", nindex++, __LINE__, nSubtractFeeFromAmount);
                // vouts to the payees
                for (const auto& recipient : vecSend)
                {
			 ...
			 printf("[%04d]:%d, nValueToSelect = %lld\n", nindex++, __LINE__, nValueToSelect);
			 			 printf("[%04d]:%d, nValueIn = %lld\n", nindex++, __LINE__, nValueIn);
                }
		...
		printf("[%04d]:%d, nValueIn = %lld\n", nindex++, __LINE__, nValueIn);
		printf("[%04d]:%d, nValueToSelect = %lld\n", nindex++, __LINE__, nValueToSelect);
		printf("[%04d]:%d, nChange = %lld\n", nindex++, __LINE__, nChange);
       	...
       	printf("[%04d]:%d, nFeeRet = %lld\n", nindex++, __LINE__, nFeeRet);
                ...
                if (nFeeNeeded < minRelayFee)
                {
                	// hul add
                	LogPrintf("[Transaction too large for fee policy] TxBytes=%d, fee needed =%d, minRelayTxFee=%d\n", nBytes, nFeeNeeded, minRelayFee);
 ...
 * https://vhkdtalk.org/?topic=54527, or
 * https://github.com/vhkd/vhkd/pull/1393.
 */
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (gArgs.GetBoolArg("-zapwallettxes", false)) {
	 printf("[CreateWalletFromFile]: zapwalletexes .\n");
	...
	//printf("first fun generate new hd master key\n");
        // generate a new master key
        CPubKey masterPubKey = walletInstance->GenerateNewHDMasterKey();
	printf("first run masterPubkey:\n");
	for(const unsigned char*pch = masterPubKey.begin(); pch != masterPubKey.end(); pch++)
		{
			printf("%c",pch[0]);
		}
	printf("\n----END-----\n");
