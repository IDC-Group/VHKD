...
void CKey::MakeNewKey(bool fCompressedIn) {
	...
	//unsigned char chout[256] = {0};
	//memcpy(chout, keydata.data(), keydata.size());
	//printf("\n[MakeNewKey]: NewKey=%s\n", chout);
	// hul
	//printf("\n[MakeNewKey] :keydata%d\n", keydata.size());
	for(int i = 0; i< keydata.size(); i++) // 32 byte  / 256bit
		{
			printf("%x", keydata[i]);	
		}
		printf("\n---------------\n");
...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
    ...
    unsigned char rnd[8];
    std::string str = "vhkdCoin key verification\n";
